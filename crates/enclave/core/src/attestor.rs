...
impl Attestor for DstackAttestor {
    ...
    fn attestation(&self, user_data: impl HasUserData) -> Result<Self::Attestation, Self::Error> {
        ...
        // The enclave produces the TDX quote. The zkdcap proof (Groth16)
        // is generated by the host after receiving this attestation, since
        // proof generation takes ~7 minutes on CPU. The host fills in
        // zkdcap_proof, zkdcap_public_inputs, and zkdcap_journal before
        // submitting the transaction to the contract.
        Ok(DstackAttestation::new(
            quote_bytes,
            response.event_log,
            ud,
            compose_hash,
            vec![],  // zkdcap_proof: filled by host
            vec![],  // zkdcap_public_inputs: filled by host
            vec![],  // zkdcap_journal: filled by host
        ))
    }
}
