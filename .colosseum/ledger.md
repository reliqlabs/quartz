...
| 8 | `commitHashE : UserDataCommit ↪ UserData` | UserDataCommit | (d) | pigeonhole-impossible (type-level visible post-cycle 6.18) | `randomOracle` over `BitVec 512` + birthday bound (cycles 6.22.a/b/c landed 2026-05-23/24; log-collision bound proven, RO-shape `commitHashCollisionAdvRO` advantage + bound proven; downstream packaging migration queued as 6.22.d pending asymptotic-framing decision) | classical chain (4 theorems); shadowed in `_negl` by `CommitHashCollisionAdv` hypothesis |
...
| 12 | `commitHashBytesE : ByteSeq ↪ UserData` | RawMessages | (d) | pigeonhole-impossible (type-level visible post-cycle 6.18) | `randomOracle` over `BitVec 512` + birthday bound (cycles 6.22.a/b/c landed 2026-05-23/24; log-collision bound proven, RO-shape `commitHashBytesCollisionAdvRO` advantage + bound proven; downstream packaging migration queued as 6.22.d pending asymptotic-framing decision) | classical chain (3 theorems); shadowed in `_negl` by `CommitHashBytesCollisionAdv` hypothesis |
