...
> 1. Round D Critical 4 production hook at `crates/contracts/core/src/handler/execute/attested.rs`:
>    - ~~`report_data` half~~ **landed 2026-05-21 (commit 88f2f08)**. The `verify_journal_binds_report_data` helper decodes `self.zkdcap_journal`, hex-decodes `report_data`, and asserts equality against `self.user_data`. Closes the "anybody-can-substitute-the-attested-user-data" vector.
>    - **`compose_hash` half — path-(c) closure landed 2026-05-21**. Added `Config::expected_rtmr3: Option<[u8; 48]>` (and matching `RawConfig` field with custom serde for the 48-byte array). When set, the production hook's new `verify_journal_bindings` helper verifies `journal.rtmr3 == config.expected_rtmr3` after the report_data binding. Deployers compute `expected_rtmr3` once from a known-good quote of the intended dstack image (e.g., the `rtmr3` field of a canonical `DcapJournal`). When `None` (backwards-compatible default), the binding is skipped and the residual "wrong-image-attestation" vector remains open — set `expected_rtmr3` to close it. Paths (a) and (b) are still cleaner end states (path (a) keeps the compose_hash semantic; path (b) avoids the per-deployer rtmr3 computation) but require cross-repo or SHA-384 work; path (c) is the cheapest interim binding with no cross-repo dep.
> 2. ~~Round D Critical 5 production deletion (policy choice A)~~ **landed 2026-05-21**. `impl Import for DefaultKeyManager` (was `default.rs:49-57`) and `impl Import for DstackKeyManager` (was `dstack.rs:164-181`) both removed; replaced in-place with policy-documentation comments referencing the Verus prototype's `*Lifecycle::import_with_rotate` for the sound add-back path. `K: Import` bound dropped from the `Backup` impl on `DefaultEnclave` in `lib.rs`. The `key_manager.import` call removed from `try_restore`; the backup file's `key_manager` field is now read-and-discarded for forward-compatibility with sealed files that predate this change. Workspace `cargo check` clean (default + wasm32 + mock features).
> 3. ~~Round E `kani_slow` CI integration~~ **landed 2026-05-21**. Extracted `LightClientOpts::validate_inputs` from `new`, returning `Result<(), &'static str>` instead of `Result<_, StdError>`. Harnesses now exercise `validate_inputs` directly, avoiding `StdError::msg`'s `Backtrace::capture()` (the unbounded path under Kani's host-arch simulation). `#[cfg(kani_slow)]` gate removed. Both harnesses verify in &lt; 0.2s under standard `cargo kani` (`light_client_opts_threshold_validation`: 26/26 checks SUCCESS; `light_client_opts_height_bounds`: 17/17 checks SUCCESS). As a side benefit, Kani caught a real `u64` overflow in the original `3 * numerator < denominator` check (would fire for `numerator > u64::MAX / 3`); fixed by lifting to `u128`.
>
...
>
> **Cycle 7.1-7.3.a 2026-05-24 (DCAP reference verifier — scaffold, substeps, soundness reduction scaffold)**: new `proofs/lean/Specs/Quartz/Attestation/DcapVerifier.lean` (~500 lines) provides the structural scaffolding for an in-Lean reference DCAP v4 verifier that will demote the bundled `axiom tdxVerifier (n : Nat) : TdxVerifier n` from a load-bearing trust assumption to a derived definition modulo three named (c)-bucket cryptographic assumptions. Wire-format types match `zkdcap/circuits/dcap-gnark/witness/quote.go` (header 48B, TDReport10 584B, AuthData variable). Collateral types: `TcbInfo`, `QeIdentity`, `Collateral`. Reference verifier `verifyDcap (n : Nat) (raw : RawBytes) (col : Collateral) : Option (MrEnclave × UserData n)` composes 7 substeps (parse, X.509 chain, QE report sig, attestation key binding, quote body sig, TCB gate, QE id gate). Bridge `dcapTdxVerifier n col fresh : TdxVerifier n` constructs a concrete TdxVerifier instance. Cycle 7.2.a implements `composeMrEnclave := body.mrTd` and `projectUserData` (deployed n=512 via cast); cycle 7.2.b implements `checkTcbLevel` (walks TCB levels for UpToDate match) and `checkQeIdentity` (bit-extracts MRSIGNER/ISVPRODID/ISVSVN from the QE report, compares to collateral). Cycle 7.3.a scaffolds the soundness reduction: introduces three named (c)-bucket assumptions (`pckLeafKey_signs_imply_signed_by_dstack`, `chain_verified_leafKey_is_legitimate`, `dcapVerifier_sound_composed`) and derives `dcapVerifier_sound` as a one-liner from the bundled `_composed` assumption (cycle 7.3.b will refine to chain-by-chain discharge). `dcapTdxVerifier`'s closure: `{propext, dcapVerifier_complete, dcapVerifier_sound_composed, was_signed_by_dstack}`. Remaining opaque substeps (parseDcapQuote, verifyX509Chain, verifyEcdsaP256, verifyAttestationKeyBinding) need substantial separate libraries (ASN.1, EC arithmetic, SHA-256/384). Cycle 7.5 (queued, multi-file): refactor `Dstack.lean` to use `dcapTdxVerifier productionCollateral productionCollateral_fresh` instead of `axiom tdxVerifier` — moves `tdxVerifier` out of the axiom closure of every downstream theorem and replaces it with the three named (c)-bucket assumptions.
>
> **Cycle 6.22.d.4 follow-up 2026-05-24 (polynomial-qb packagings)**: four new `*_secure_of_*_bundle_secure_RO_polyQb` packagings parallel the constant-qb variants but take a per-`n` query budget bounded by a polynomial — matching the real PPT adversary model. Each discharges the hash collision-resistance hypothesis via `commitHashCollisionAdvRO_negligible_of_polynomial_qb`. Closure: `{propext, Classical.choice, Quot.sound}` only. Cryptographically meaningful shape now available for any consumer that wants to thread polynomial-bounded RO adversary advantage through a protocol-layer lift's union bound.
>
> **Cycle 7.3.a refinement 2026-05-24 (named axioms gain witness-bearing types)**: the three cycle-7.3.a placeholder axioms (`pckLeafKey_signs_imply_signed_by_dstack`, `chain_verified_leafKey_is_legitimate`) had bodies returning `True` (zero content). Refined to carry meaningful witnesses via three abstract predicates (`signed_by_pck_holder`, `legitimate_pck_leaf`, `pck_holder_is_dstack_tee`). The named axioms now have real witness-bearing conclusions but are not yet load-bearing in `dcapVerifier_sound`'s closure — cycle 7.3.b will refine substep return types so the chain becomes structurally derivable. Documented honestly in `dcapVerifier_sound_composed`'s docstring.
>
> **Cycle 6.22.d.4 follow-up 2026-05-24 (polynomial-qb negligibility — finding #15)**: `commitHashCollisionAdvRO_negligible_of_polynomial_qb` and the byte-domain analogue close the previously-open case of polynomial-in-n query budgets (the constant-qb version was insufficient for real PPT adversaries whose query count scales with the security parameter). Composes the parameterised birthday bound with VCV-io's `negligible_polynomial_mul` + `negligible_inv_two_pow`. Closure: `{propext, Classical.choice, Quot.sound}` only. Implementation note: had to thread `Nat` instead of `ℕ` through the signature to avoid Lean's type-inference confusion with `Uint128 := Nat` from TransferMessages.lean — Polynomial type-class resolution was matching `Polynomial Uint128` instead of `Polynomial ℕ` in some elaboration contexts. Auditable workaround; future cycles may want to rename `Uint128` or scope its abbrev.
>
> **Cycle 6.22.d.4 2026-05-24 (adversarial-review response — 4 critical findings addressed via docstring-honesty)**: `colosseum-spec-adversary` review of the cycle-6.22.d.3 parameterisation returned 26 findings (4 critical, 13 serious, 9 cosmetic). Headline: the `commitHashE n` axiom family is constructively-False at `n = 0` (BitVec 0 has one inhabitant; `Inhabited UserDataCommit` plus any second value derives False directly), AND the four `*_secure_via_RO_packaging` connection theorems use `qb := 0` zero-query adversaries and zero-advantage tdxExp/circuitEqExp to discharge the packaging — making the "discharge collision-resistance internally" claim theatrical. This cycle landed docstring-level fixes: (1) `commitHashE` gains a multi-paragraph **Safe-use convention (n ≥ 1)** note acknowledging the constructive-False at `n = 0` and explaining the constraint is documentary rather than type-enforced (the `[NeZero n]` typeclass cascade across ~20 files / ~80 sites was deemed too expensive for the in-fork audit story where no consumer instantiates at `n = 0`); (2) module-level docstring of `ProtocolVCVioROModel.lean` reframed to honestly characterise the four connection theorems as "type-shape compatibility demonstrations" rather than substantive cryptographic wirings, including an explicit note about the `simulateQ (protocolSpecHonestSim n)` (deterministic) vs `simulateQ loggingOracle` (uniform) probability-space mismatch; (3) `commitHashCollisionAdvRO_deployed_bound` docstring retracts the "well within the negligibility envelope" overclaim and adds "informative only when `qb² ≪ 2^512`" honesty. Build green at 2676 jobs; 0 new `sorry`. 11 findings deferred (mostly cosmetic or same-scope as the typeclass cascade); 12 findings closed via the docstring fixes; 3 findings explicitly characterised as "known limitations of the documentary-convention approach". Methodology v0.4 ask candidate added: when parameterising an abstract injectivity/embedding axiom by a security parameter, audit the family for degenerate-parameter inconsistencies pre-emptively. Change record: `.colosseum/changes/2026-05-24T04-00-00Z-cycle-6.22d4-adversarial-review-response.md`. Full adversarial-review output is captured inline in that change record.
>
> **Cycle 6.22.d.3 2026-05-24 (wholesale spec parameterisation — LANDED via aggressive replacement)**: maintainer chose option (a)/aggressive replacement. `UserData : Type := BitVec 512` refined to `UserData (n : Nat) : Type := BitVec n` across the whole `Specs/Quartz/*` tree. `commitHashE`, `commitHashBytesE` axioms became axiom families `commitHashE (n) : UserDataCommit ↪ UserData n`; `tdxVerifier` became `tdxVerifier (n) : TdxVerifier n`; `commitHash`, `commitHashBytes`, `pkOfUserData`, `userDataOf{TransferRequest,ResolveMessage,SessionCreate,SessionSetPubKey}`, `verifyTdxQuote`, `mrEnclaveOf`, `userDataOf`, `verifyTdxQuote_sound`, `verifyTdxQuote_complete`, `verifyGroth16_yields_decoded`, etc. all gained explicit `(n : Nat)` parameters; the protocol-layer `ProtocolSpec n`, `protocolSpecHonestSim n`, all five adversary types (`Groth16SoundAdv`, `HandshakeSoundAdv`, `TdxVerifierSoundAdv`, `HandshakeBindsAdv`, `SessionConfidentialityAdv`, `SessionConfidentialityExtractorAdv`, `TransfersConservationAdv`, `AuctionDeterminismAdv`, `Groth16KSAdv`, `CircuitEqAdv`, `CrossSessionBindAdv`, `CommitHashCollisionAdv`, `CommitHashBytesCollisionAdv`) became `(n : ℕ) → OracleComp (ProtocolSpec n) (T n)` shapes with `T : ℕ → Type`. `IsPPT_proper` generalised from `T : Type` to `T : ℕ → Type` to handle adversaries whose output type depends on the security parameter (e.g., `HandshakeCheck n`). All eight cycle-6.4-6.11 lift proofs re-verified under the parameterised carriers. The cycle 6.22.a-d.2.5 work consolidated into a single `ProtocolVCVioROModel.lean` (dropping the redundant `_N` suffix variants now that the underlying carriers are natively parameterised). Build green at **2675 jobs**; 0 new `sorry`. The cycle 6.22 RO infrastructure remains intact: birthday bound, RO-shape advantages, asymptotic negligibility, four RO-form packagings (with internal collision-resistance discharge), four connection theorems. The constant query budget `qb` gives a negligible advantage `qb²/(2·2^n)` super-polynomially decaying in `n` — the framing tension from cycle 6.22.c is fully resolved.
>
> **Cycle 6.22.d.3 2026-05-24 (wholesale spec parameterisation — pre-landing notes, staging-design analysis)**: cycle 6.22.d.1/.2/.2.5 landed the option-(a) foundation. Wholesale parameterisation of the protocol-layer adversary types and existing lift theorems is **staging-design blocked** (not structurally blocked). Three coordination options documented in `.colosseum/changes/2026-05-24T02-00-00Z-cycle-6.22d3-design-blocker.md`: aggressive in-place replacement (~1 week, single big-bang), conservative parallel (~2-3 weeks, low risk), hybrid (~1-1.5 weeks, probably correct shape). Each choice affects every consumer of `Specs/Quartz/*`. Maintainer's call. Methodology v0.4 ask: distinguish "structural blocker" from "staging-design blocker" in change records.
>
> **Cycle 6.22.d.1/d.2/d.2.5 2026-05-24 (option-(a) parameterised RO + four RO-form packagings + four connection theorems)**: `proofs/lean/Specs/Quartz/Protocol/ProtocolVCVioROModelParam.lean` provides the security-parameter-indexed RO model: `UserDataN n := BitVec n`, `CommitHashSpecN n` / `CommitHashBytesSpecN n`, parametric typeclass instances, parameterised log-collision birthday bounds, RO-shape adversary types `CommitHashCollisionAdvRO_N` / `CommitHashBytesCollisionAdvRO_N`, advantage definitions, parameterised bounds (`≤ qb²/(2·2^n)`), and crucially the **asymptotic negligibility lemma** `commitHashCollisionAdvRO_N_negligible_of_constant_qb : ∀ qb constant, negligible (commitHashCollisionAdvRO_N 𝒜)` (and the bytes-domain analogue). Auxiliary lemma `negligible_inv_two_pow : negligible (fun n => (2^n)⁻¹)` lifted from Mathlib's `tendsto_pow_const_div_const_pow_of_one_lt` — a foundational asymptotic-decay lemma the project didn't previously have. Then cycle 6.22.d.2 adds four RO-form packagings (`handshakeBindsFail_*_RO`, `sessionConfFail_*_RO`, `sessionConfExtractor_*_RO`, `crossSessionBindFail_*_RO`) that take a `CommitHashCollisionAdvRO_N` adversary plus query budget and **discharge the hash negligibility hypothesis internally** via the birthday bound — the first packagings in the project that don't take collision-resistance as an assumed hypothesis, deriving it instead. All theorems close on `{propext, Classical.choice, Quot.sound}` only. Build green at 2676 jobs (+6 from cycle 6.22.c). The cycle-6.22.c framing tension is resolved: the parameterised advantage IS asymptotically negligible. Change record: `.colosseum/changes/2026-05-24T01-00-00Z-cycle-6.22d1-d2-parameterised-ro.md`. Remaining cycle 6.22.d.3+ work: migrate the protocol-layer adversary types (`HandshakeBindsAdv`, etc.) and lift theorems to parameterised carriers — wholesale refactor of `ProtocolVCVio.lean` and the four lift files, multi-day scope.
>
> **Cycle 6.22.c 2026-05-24 (RO-shape advantage + bound; cycle 6.22.d blocked on framing decision)**: `proofs/lean/Specs/Quartz/Protocol/ProtocolVCVioROModel.lean` now provides RO-shape adversary types `CommitHashCollisionAdvRO`, `CommitHashBytesCollisionAdvRO` (each `ℕ → OracleComp <spec> Unit`), corresponding advantage definitions (`Pr[LogHasCollision z.2 | (simulateQ loggingOracle (𝒜 n)).run]`), and birthday bounds (`commitHashCollisionAdvRO 𝒜 n ≤ qb²/(2·2^512)` for query-budgeted adversaries) — both bound theorems close on `{propext, Classical.choice, Quot.sound}` only. Cycle 6.22.d (downstream packaging migration) is **blocked on a design decision**: the bound is a *constant* in the security parameter `n`, while the protocol-layer lift packagings (`*_secure_of_*_bundle_secure`) consume asymptotic negligibility. Fixed-size hash carriers (`UserData = BitVec 512` from cycle 6.18) and asymptotic-security framing are incompatible. Three options documented: (a) re-parameterize the spec by security parameter (1-2 weeks, only path to genuine asymptotic security), (b) re-frame packagings to expose concrete advantage bounds instead of `.secure` conclusions (2-3 days, honest about fixed-size hash), (c) add a fake "hash advantage is negligible" axiom (regression, not recommended). Change record: `.colosseum/changes/2026-05-24T00-00-00Z-cycle-6.22c-ro-advantage-bound.md`. New methodology v0.4 ask candidate: spec-time pre-flight check for fixed-size-carrier / asymptotic-negligibility mismatch.
>
...
- Build status: `lake build` green at 2670 jobs (cycle 6.13 added the `SimSemantics.Append` transitive import); **10 axioms** from the original 26-axiom inventory remaining (cycles 6.16–6.21 refined 12 carrier-type axioms to concrete types and demoted 3 (a)-bucket named-constant axioms to defs; the surviving 10 are: 4 (d)-bundle assumptions, 2 serialize-injection genuine claims, 2 bridge equalities, 1 function signature `keyOf`, 1 off-chain witness `was_signed_by_dstack`); cumulative form-phase reduction 40 → 10 = **–75%**; `verus` across the prototype tree green (55 verified, 0 errors total: attested 14, instantiate 8, session_create 6, session_set_pub_key 7, encryption 6, key_manager 14 post-Critical-5-substantive). Round D Criticals 1, 2, 3, 4-Verus-side, and 5 (substantive) all closed by 2026-05-20.
...
| 8 | `commitHashE : UserDataCommit ↪ UserData` | UserDataCommit | (d) | pigeonhole-impossible (type-level visible post-cycle 6.18) | `randomOracle` + parameterised birthday bound (cycles 6.22.a-d.2 landed 2026-05-23/24; fixed-size + parameterised RO advantages, both with proven bounds and asymptotic negligibility; four RO-form packagings discharge collision-resistance internally; protocol-layer lift migration to parameterised carriers queued as 6.22.d.3+) | classical chain (4 theorems); shadowed in `_negl` by `CommitHashCollisionAdv` hypothesis (legacy honest-det path); new RO-form packagings consume `CommitHashCollisionAdvRO_N` with internal discharge |
...
| 12 | `commitHashBytesE : ByteSeq ↪ UserData` | RawMessages | (d) | pigeonhole-impossible (type-level visible post-cycle 6.18) | `randomOracle` + parameterised birthday bound (cycles 6.22.a-d.2 landed 2026-05-23/24; fixed-size + parameterised RO advantages, both with proven bounds and asymptotic negligibility; quad-bundle RO-form packaging discharges collision-resistance internally; protocol-layer lift migration queued as 6.22.d.3+) | classical chain (3 theorems); shadowed in `_negl` by `CommitHashBytesCollisionAdv` hypothesis (legacy); new quad packaging consumes `CommitHashBytesCollisionAdvRO_N` with internal discharge |
...
| 8 | `commitHashE` | (d) | pigeonhole-impossible; cycle 6.22.b/c (log-collision bound + RO advantage definition + bound) proven 2026-05-23/24; 6.22.d packaging migration pending asymptotic-framing decision |
| 10 | `serializeRawSessionCreateE` | (c) | serde_json injectivity |
| 11 | `serializeRawSessionSetPubKeyE` | (c) | serde_json injectivity |
| 12 | `commitHashBytesE` | (d) | pigeonhole-impossible; cycle 6.22.b/c (log-collision bound + RO advantage definition + bound) proven 2026-05-23/24; 6.22.d packaging migration pending asymptotic-framing decision |
...
1. **Cycles 6.12–6.14** (deeper refactors than def-tying):
   ...
   - 6.17 – 6.20 **(done 2026-05-20)**: carrier refinement batch. `MrEnclave` → `BitVec 384` (6.17, Intel TDX MRTD 48-byte SHA-384 digest); `UserData` → `BitVec 512` (6.18, DCAP report_data 64-byte slot — highest-leverage refinement, provides `Fintype UserData` which unblocks the `commitHashE` random-oracle discharge); `PrivKey` → `BitVec 256`, `PubKey` → `BitVec 264`, `Plaintext` → `List UInt8` (6.19, ECIES carriers); `DomainSep` → `List UInt8`, `Addr` → `String`, `ByteSeq` → `List UInt8` (6.20, remaining carriers). Same cycle 6.20 also demoted the three (a)-bucket named-constant axioms (`rawDomainSep`, `rawBoundContract`, `rawPlaceholderPubKey`) to concrete `def`s with documentary values (`"QUARTZ-HS-V1".toUTF8.toList`, `"xion1quartzdeploymentaddress"`, `0 : BitVec 264`). Original-inventory axiom count: 25 → 14. (a)-bucket count: 3 → 0. (c)-bucket count: 18 → 10. (d)-bucket count: unchanged at 4 (the four bundle axioms still require external or random-oracle discharge). Build green at 2670 jobs across all four cycles, zero new sorry. Honesty caveat: `commitHashE` and `commitHashBytesE` are now visibly pigeonhole-impossible at the type level (`UserDataCommit` infinite via `List UInt8`/`String` fields, codomain `BitVec 512` finite); no existing proof attempts to use this to derive `False`. Substantive Option-(a) closure of (d-pigeonhole-impossible) via VCV-io's `randomOracle` + birthday bound is now tractable and queued as cycle 6.22. Change record `.colosseum/changes/2026-05-21T00-40-00Z-cycles-6.17-6.20-carrier-refinement-batch.md`.
   - 6.21 **(done 2026-05-20)**: attestation wire-format carrier refinement. `TdxQuote`, `Groth16Proof`, `PublicInputs`, `VKey` all refined from `axiom ... : Type` to `abbrev ... : Type := List UInt8`. The four are variable-length byte sequences at the spec level — modelling them as `List UInt8` is faithful to the wire-format reality (DCAP-quote-v4 has variable PCK chain inlining; Groth16 vkeys vary by public-input count; PublicInputs is concatenated fr.Element bytes). Original-inventory axiom count: 14 → 10. Cumulative session reduction: 26 → 10 (cumulative from baseline 40 → 10 = –75%). Build green at 2670 jobs. Change record `.colosseum/changes/2026-05-21T01-00-00Z-cycle-6.21-attestation-carrier-refinement.md`. Substantive Option-(a) closure of `commitHashE`/`commitHashBytesE` via VCV-io `randomOracle` + birthday bound queued as cycle 6.22.
   - 6.22.a **(done 2026-05-23)**: random-oracle simulator scaffolding. New module `Specs/Quartz/Protocol/ProtocolVCVioROModel.lean` (~267 lines) with `commitHashROSim`/`commitHashBytesROSim` via VCV-io's `randomOracle`, plus the supporting `DecidableEq UserDataCommit`/`DecidableEq ByteSeq` and `SampleableType (CommitHashSpec.Range t)`/`SampleableType (CommitHashBytesSpec.Range t)` instances. Build green at 2670 jobs. Zero new `sorry`. Change record `.colosseum/changes/2026-05-23T00-00-00Z-cycle-6.22-ro-scaffolding-and-blocker.md`.
   - 6.22.b **(BLOCKED 2026-05-23)**: birthday-bound theorem for `commitHashCollisionAdv`. Blocked by VCV-io's `probEvent_logCollision_le_birthday_total` requiring `[spec.DecidableEq] [spec.Fintype] [spec.Inhabited]`, where `OracleSpec.Fintype` extends `PFunctor.Fintype` and requires `Fintype` on BOTH the index AND range types. `CommitHashSpec : OracleSpec UserDataCommit` has `Fintype`-ok range (`BitVec 512`) but `Fintype`-fails index (`UserDataCommit` contains `List UInt8`/`String` fields post cycle 6.20). Genuine incompatibility between cycle 6.21's production-faithful variable-length carrier choices and VCV-io's fixed-size-index assumption. Three remediation paths documented in the in-file blocker note: (a) bounded-variant spec `BoundedCommitHashSpec` over `Vector UInt8 N × ...` — recommended, ~2-3 days; (b) upstream VCV-io PR to drop `[spec.Fintype]` on index — multi-week; (c) revert cycle 6.21 to fixed-width carriers — not recommended (regresses production correspondence). Cycle 6.22.c is conditional on 6.22.b.1 landing.
