...
| Verus | 6 prototypes (feasibility specs, post-Round-D-hardened with two known follow-up scopes) | 46 verified, 0 errors across the tree (Round D Criticals 1, 2, 3, 4-Verus, 5-cheap-fix all closed by 2026-05-20) | 38 unsampled annotations; Critical 4 production-side and Critical 5 substantive remediation queued as follow-ups (see banner) | post-Round-D + cross-critique: see `.colosseum/attacks/verus-prototype-2026-05-14/synthesis.md` and `.colosseum/attacks/verus-prototype-cross-critique-2026-05-20/synthesis.md` |
...
**[REVISED 2026-05-20 after Round D + cross-critique + spec-side closures]** All Round D blockers are now either closed or partitioned into named follow-up scopes. Critical 4 production-side is queued for the Quartz agent at `crates/contracts/core/src/handler/execute/attested.rs:94-99` (decode `zkdcap_public_inputs` or `zkdcap_journal`, verify-equal encoded `report_data` and `compose_hash` against the wrapper-validated fields). Critical 5 substantive remediation is queued as a Colosseum-agent follow-up cycle (model `DefaultKeyManager` import as mutation, model `DstackKeyManager`, add session-lifecycle ghost layer, decide production key-rotation policy). Both are scoped at `.colosseum/attacks/verus-prototype-cross-critique-2026-05-20/synthesis.md`.
