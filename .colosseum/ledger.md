...
1. **Cycles 6.12–6.14** (deeper refactors than def-tying):
   ...
   - 6.14 (queued): replace placeholder `IsPPT := True` with `IsPPT := PolyQueries`, closing Round A attack #5 substantive side; AND (separately) replace `pkOfUserData_commitHash`'s reliance on `commitHash_inj` with a probabilistic collision-resistance hypothesis — closes Round A attack #8 and surfaces a real second bundle at the terminal lift.
