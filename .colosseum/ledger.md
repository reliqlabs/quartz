...
> 1. ~~Round D Critical 4 production hook at `crates/contracts/core/src/handler/execute/attested.rs:94-99`~~ **report_data half landed 2026-05-21 (parallel-tracks A/B/C cycle, commit pending)**. The `verify_journal_binds_report_data` helper now decodes `self.zkdcap_journal`, hex-decodes `report_data`, and asserts equality against `self.user_data`. The compose_hash binding remains queued separately because `DcapJournal` does not expose `compose_hash` directly (only `rtmr3` 48-byte SHA-384). Two paths: (a) extend `zkdcap_core::DcapJournal` to include `compose_hash: [u8; 32]` (cross-repo PR), (b) on-chain RTMR3-extension verifier (expensive, requires initial-RTMR-3 state in Config). See `.colosseum/changes/2026-05-21T01-30-00Z-parallel-tracks-A-B-C.md`.
