...
- Build status: `lake build` green at 2667 jobs (Step 6.3 baseline); `verus` across the prototype tree green (46 verified, 0 errors total: attested 14, instantiate 8, session_create 6, session_set_pub_key 7, encryption 6, key_manager 4 post-tautology-removal). Round D Criticals 1, 2, 3, 4-Verus-side, 5-cheap-fix all closed by 2026-05-20.
- Phase: ~~end of VCVio refactor Steps 0-6.3 (form-phase axiom reduction 40 → 26 = -14 / -35%; content-phase lift of all 8 protocol theorems complete)~~ — **[UPDATED 2026-05-20]** form-phase reduction holds (40 → 26 axioms); content-phase lift sequence (cycles 6.4–6.11) complete, all 8 `_negl` theorems def-tied with content-bearing `Pr[…]`-based advantages and proven (not assumed) pointwise bounds. Refactor is at "form-phase + def-tying content-phase complete; carrier-refinement / IsPPT-PolyQueries / Option-(b)-commitHash content-phase pending (cycles 6.12–6.14); Verus-prototype Round D Criticals 1–3 and the Verus-side of 4 closed, Critical 5 reduced to a docstring-honest snapshot binding + a queued lifecycle-modeling follow-up cycle, Critical 4 production-side queued for Quartz agent". Ready to draft upstream PR with Critical 4 production-side and Critical 5 substantive remediation called out as known follow-up scopes.
...
| Verus | 6 prototypes (feasibility specs, ready to ship in upstream PR with two known follow-up scopes) | 46 verified, 0 errors across the tree (Round D Criticals 1, 2, 3, 4-Verus, 5-cheap-fix all closed by 2026-05-20) | 38 unsampled annotations; Critical 4 production-side and Critical 5 substantive remediation queued as follow-ups (see banner) | post-Round-D + cross-critique: see `.colosseum/attacks/verus-prototype-2026-05-14/synthesis.md` and `.colosseum/attacks/verus-prototype-cross-critique-2026-05-20/synthesis.md` |
...
5. Critical 5 (`pub_key_matches_sk` does not survive `Import::import`) — **refined and partially closed 2026-05-20 in commit `3267015`**. Cross-critique reached 3-of-5 THIRD_OPTION with convergent refinements from Claude, GPT-5.5, and Kimi. The cheap-honest part landed: the tautological `pub_key_matches_sk` theorem (whose `ensures` was `forall k. f(k.sk) == f(k.sk)`) is deleted and replaced with a docstring redirect to where the snapshot binding actually lives. **The substantive remediation is queued as a Colosseum-agent follow-up cycle**: (a) model `DefaultKeyManager` import as a mutating operation, (b) model `DstackKeyManager` (production default per `CLAUDE.md`, currently unmodeled per Round D Critical 19), (c) add a session-lifecycle ghost layer with a `contract_pub_key` field and invariants tying it to `km.sk` across state transitions, (d) decide production key-rotation policy (remove `Import` impls vs. add a `session_rotate_pub_key` contract message). See `.colosseum/attacks/verus-prototype-cross-critique-2026-05-20/synthesis.md` for the full refined plan.
