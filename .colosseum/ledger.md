...
>
> **Remaining Quartz-agent follow-ups (both surfaced by the Colosseum side; neither blocks further Colosseum-side work)**:
...
> 2. Round E `kani_slow` CI integration — replace `StdError::msg` with backtrace-free constructor under `#[cfg(kani)]` so the LightClientOpts validation harnesses become tractable under standard `cargo kani`. Also unlocks the Kimi #5 `user_data` determinism harnesses (same underlying tractability issue).
>
