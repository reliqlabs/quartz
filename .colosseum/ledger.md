...
| Verus | 6 prototypes (feasibility specs, **NOT** integration-ready per Round D 2026-05-14/15) | 5 of 43 verified, with Critical 1 unsoundness closed 2026-05-20 (`ec24934`) | 38 unsampled annotations; 4 Round D criticals open (Criticals 2–5: SEQUENCE_NUM model gap, Attested compensating variant missing, ZK input-binding, key-manager import temporal binding) | post-Round-D: see `.colosseum/attacks/verus-prototype-2026-05-14/synthesis.md` |
...
### Round D Verus-prototype blockers (queued for upstream-PR readiness)

Critical 1 (unsound `signing_key_bytes_roundtrip_axiom`) closed 2026-05-20 in commit `ec24934`. Four blockers remain. The upstream-PR readiness criterion from the methodology stance is that the prototypes' verification claims must not contain known soundness breaks, model gaps with universal cross-voice agreement, or docstring promises that the file fails to honor.

1. Critical 2 (`session_set_pub_key.rs` does not model `SEQUENCE_NUM` reset). Mechanical fix: add `sequence_num: Option<u64>` to the prototype's `Storage` struct, extend the Ok postcondition with `final(storage).sequence_num == Some(0)`, add an atomicity invariant covering the partial-first-save case. Six voices flagged this; resolving it closes the highest-agreement Round D finding.
2. Critical 3 (`Attested<M,A>::handle` compensating variant missing). Two options: re-add the promised `concrete_att_handle_maybe_err` external_body variant in `attested.rs` and prove inner-handler error propagation, or strike the docstring claim at lines 43–46 and label the prototype as silent on inner-handler failures. Either path closes the credibility issue. The first is the substantive fix; the second is the honest-relabel.
3. Critical 4 (DstackZk `public_inputs` not bound to wrapper `compose_hash`/`user_data`). The Verus omission mirrors a production omission. Fix path: add a spec-level uninterpreted predicate `proof_journal_binds(proof, public_inputs, expected_compose_hash, expected_user_data)` and require it on the Ok branch. The production handler then needs the corresponding extract-or-verify-equal check before accepting the proof. The Verus side is straightforward once the production fix path is decided; the upstream PR description should call this out as an item the prototypes do not cover and that the production code should also fix.
4. Critical 5 (`pub_key_matches_sk` does not survive `Import::import`). Fix path: add a `pub_key_published` ghost field to `DefaultKeyManager`, prove `import` either invalidates it (sets to `None`) or atomically updates it. This is the temporal binding property the current snapshot-only theorem does not capture.
...
**[REVISED 2026-05-20 after Round D + Critical 1 fix]** The single highest-leverage near-term work is closing the four open Round D Verus-prototype blockers (Criticals 2 through 5 above) before drafting the upstream PR. The ledger's audit-ready paragraph is the spine of that PR description per `CLAUDE.md:27`, and shipping a PR while a "verified" prototype directory carries a docstring promise the file fails to honor (Critical 3) or a model gap with universal cross-voice agreement (Critical 2) is a credibility risk. Criticals 4 and 5 are Claude-arm-only findings; the v0.4 cross-critique cycle is the natural next step for them, ideally with the newly-available OpenAI voice added to the opencode dispatch lineup (per Quartz `CLAUDE.md:30-36` and `verified-rcv/.colosseum/scripts/opencode_dispatch.py:242` voice list).

After the Round D blockers, the previously-identified near-term Quartz-side item stands: demoting `rawDomainSep`, `rawBoundContract`, `rawPlaceholderPubKey` to `def`s once their carriers are refined to concrete byte strings. That refactor would close out the (a) bucket entirely. Methodology-side, the v0.2 back-port has progressed past the Round-A meta-finding into v0.4 candidates O through T at `colosseum/methodology-v0.4-candidates.md` (cross-critique as standard post-fan-out, defense round, intent-tightening, re-cross-critique, ghost-variable encoding, `--variant high` default for spec-class dispatch). The upstream Quartz discharges (concrete hash spec, ArkLib integration, reference DCAP verifier, carrier refinement) remain upstream-blocked at the same cadence as before.
