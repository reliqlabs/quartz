...
1. **Cycles 6.12–6.14** (deeper refactors than def-tying):
   ...
   - 6.14.b/c (queued): per-reduction `IsPPT_proper`-preservation lemma showing the `bind/pure ∘ proj` shape adds no oracle queries; new `_AGAINST_PPT_ADVERSARIES` packagings consuming `IsPPT_proper`. Closes Round A attack #5 substantive side.
   - 6.15 (queued): replace `pkOfUserData_commitHash`'s reliance on `commitHash_inj` with a probabilistic collision-resistance hypothesis — closes Round A attack #8 and surfaces a real second bundle at the terminal lift.
