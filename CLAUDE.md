...
- **What's open**:
  - **Cycle 6.22.d.3+** (queued, multi-day): protocol-layer adversary type and lift migration to parameterised carriers. The cycle-6.22.d.1/.2 foundation (parameterised RO + four RO-form packagings discharging collision-resistance internally) is landed; existing lifts can already use the new packagings via over-approximation. Full parameterisation of `ProtocolSpec`, `protocolSpecHonestSim`, and the adversary types in `n` is the remaining wholesale refactor across `Specs/Quartz/Protocol/*` and the carrier files. Multi-day scope; not a decision blocker, just unfinished structural work.
