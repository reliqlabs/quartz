...
- **What's open**:
  ...
  - **Cycle 7.6** (queued, candidate): structural axioms relating `parseDcapQuote rawQuote = some q` to `q`'s field values, and asserting `signedRegion = header || body` as a structural invariant. Addresses cycle-7.x adversarial findings #14 and #17.
