...
**Status: Steps 1-7 + cycles 6.4-6.22.d.4 + cycles 7.1-7.3.b complete. The wholesale parameterisation of the verification surface (cycle 6.22.d.3) and the DCAP reference verifier scaffold (cycle 7.x) both landed 2026-05-24. Adversarial review of the parameterisation surfaced 26 findings, 4 critical, all addressed (cycle 6.22.d.4). Polynomial-qb negligibility lemma closes finding #15. DCAP scaffold (`proofs/lean/Specs/Quartz/Attestation/DcapVerifier.lean`, ~650 lines) provides wire-format types, reference verifier algorithm composition, structural substeps (composeMrEnclave, projectUserData, checkTcbLevel, checkQeIdentity), bridge to TdxVerifier, and a derived `dcapVerifier_sound_composed` theorem (cycle 7.3.b) whose closure depends on three named (c)-bucket cryptographic axioms (PCK leaf legitimacy, ECDSA-P256 EUF-CMA, verified-chain → dstack-signed). Cycle 7.x adversarial review surfaced 20 findings; 8 fixed (cycles 7.3.b + docstring honesty), 12 deferred to cycle 7.5 / cycle 7.2.c. Build green at 2676 jobs throughout; 0 new `sorry`.**
...
- **What's open**:
  - **Cycle 7.5** (queued, multi-file): refactor `Dstack.lean` to replace `axiom tdxVerifier (n : Nat) : TdxVerifier n` with `noncomputable def tdxVerifier (n) := dcapTdxVerifier n productionCollateral productionCollateral_fresh`. Moves `tdxVerifier` out of the axiom closure of every downstream theorem; replaces it with the three named (c)-bucket assumptions (`pckLeafKey_signs_imply_signed_by_dstack`, `chain_verified_leafKey_is_legitimate`, `dcapVerifier_sound_composed`). Touches every downstream theorem's axiom closure — substantive audit change.
