...
**Status: Steps 1-7 + cycles 6.4-6.22.d.2 complete. Audit-ready ledger in place; cycles 6.22.a-d.2 (RO model + log-collision bound + advantage definition + parameterised + asymptotic negligibility + four RO-form packagings) proven 2026-05-23/24 with `{propext, Classical.choice, Quot.sound}` axiom closure only; the four new packagings discharge collision-resistance internally rather than assuming it (first in the project). Cycle 6.22.d.3+ (protocol-layer adversary/lift migration to parameterised carriers) queued as multi-day refactor; awaiting external discharge paths for the (d)-bucket TDX/Groth16 axioms.**
...
- **What's open**:
  - **Cycle 6.22.d.3** (staging-design blocker): wholesale parameterisation of the protocol-layer adversary types and lift theorems. Foundation is landed (cycle 6.22.d.1/.2/.2.5: parameterised RO + four RO-form packagings + four connection theorems wiring existing lifts). The remaining refactor needs a staging-design decision from the maintainer between three options: aggressive in-place replacement (~1 week single big-bang), conservative parallel tree (~2-3 weeks, low risk), or hybrid (carrier replaced + parallel lift theorems, ~1-1.5 weeks). All three are mechanically tractable; the choice affects every consumer of `Specs/Quartz/*`. Documented in `.colosseum/changes/2026-05-24T02-00-00Z-cycle-6.22d3-design-blocker.md`.
