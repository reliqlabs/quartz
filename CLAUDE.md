...
- **zkdcap verifier migration → IMPLEMENTED, NETWORK ACTIVATION BLOCKED (Noir/UltraHonk)**: attestation uses `/xion.zk.v1.Query/ProofVerifyUltraHonk`, built on the shared `quartz-zkdcap` crate. The packed 672-byte / 21-field `public_inputs` replace the old gnark journal. The consumer sends `expected_vkey_sha256` and requires the response's `vkey_sha256` to match exactly. **The blocker is now identified precisely:** that binding exists in the `xion` tree only on branch `zk-ultrahonk-vkey-hash` (`e14a4ba`, 2026-07-17), which is in no tag (latest is `v30.0.0`) and no other branch, and `xiond` 29.0.0's `query zk verify-ultrahonk` has no digest flag at all. It must merge, release, and reach the target network before a non-mock attestation can succeed live. Relaxing the pin to name-only resolution is not an option: an Xion server that ignores the request field must fail closed. See `crates/zkdcap/`, `crates/contracts/core/src/handler/execute/attested.rs`, `crates/cli/src/handler/zkdcap.rs`, `tests/integration/src/zk_mock.rs`.
- **UltraHonk vkey on testnet**: `dcap-ultrahonk-v1` (registered; shared with dossier/verified-rcv). Circuit + prover live in `zkdcap/circuits/dcap-noir` + `zkdcap/noir-prove-server`. **That name and id are now legacy and must not be reused.** zkdcap has closed all four step-5 relation defects (`e7002e4`) and verified the current key live on `xion-testnet-2` as a **scratch** registration by name, which proves the proof format is accepted but exercises no digest path. Production registration under a versioned name in scope `zkdcap-tdx-v4-tdreport10-21` is still ahead, and five different 32-byte values in that tree currently look like a pin. Do not pin from a commit message or a build artifact; see `.colosseum/boundaries/zkdcap--quartz.md` section 4.
