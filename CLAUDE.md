...
**Status: Steps 1-7 + cycles 6.4-6.22.d.3 complete. The wholesale parameterisation of the verification surface by security parameter `n` landed 2026-05-24 (option (a)/aggressive replacement). Every Specs/Quartz/* theorem now threads `n` through carriers, axioms, simulators, adversary types, and lift theorems. Build green at 2675 jobs; 0 new `sorry`. The cycle 6.22 RO infrastructure (parameterised birthday bound + RO-shape advantages + asymptotic negligibility + four RO-form packagings discharging collision-resistance internally + four connection theorems) is consolidated into a single `ProtocolVCVioROModel.lean`. Awaiting external discharge paths for the (d)-bucket TDX/Groth16 axioms.**
...
- **What's open**:
  - **External discharge paths** for the remaining (d)-bucket axioms (`tdxVerifier`, `groth16Verifier`): need ArkLib Groth16 KS reduction (upstream-paced) and a Lean reference DCAP verifier (separate substantive effort). The cycle 6.22 RO foundation is in place; consumers of `commitHashE`/`commitHashBytesE` can now switch to the cycle-6.22 RO-form packagings to discharge collision-resistance from the birthday bound.
